...
26-tool surface (21 locked 2026-04-21 + dm_init follow-up + 3 NPC roster
tools shipped 2026-04-22 + dm_mine_status deprecated alias). See
`daimon/mcp/server.py` for the authoritative docstring + tool signatures.
High-level groups:
  ...
  PvP (arena)   dm_pvp_challenge, dm_pvp_accept, dm_pvp_status,
                dm_pvp_my_matches
  Arena state   dm_leaderboard, dm_my_rank
  Disputes      dm_dispute_open, dm_card_propose

Tools that need arena/cards-repo wiring (`dm_register`, `dm_pvp_*`,
`dm_leaderboard`, `dm_my_rank`, `dm_dispute_open`, `dm_card_propose`)
currently return a `status: not_yet_implemented` envelope that documents
the issue_shape they will use once wiring lands.
