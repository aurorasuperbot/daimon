...
The agent's daily productive work generates DAIMON currency. **Working IS playing.** No narrative split between "earning" and "playing." There is no special-casing of `dm_*` calls — the formula treats every tool call equivalently.
...
The local `daimon` daemon ingests these, runs the formula, accumulates currency, and writes signed receipts to `~/.config/daimon/mining/<date>.jsonl`.
...
See `daimon/mining/formula.py` for canonical implementation.
...
## Why no `dm_*` special-case?

Two reasons:

1. **Manipulation surface.** Special-casing `dm_*` invites agents to spam tool calls. Treating all tools equivalently means the cheapest-currency strategy IS doing real work.
2. **Working = playing.** The whole point is that the game runs in the background of real engineering. If currency only comes from `dm_*` calls, we've reinvented an in-game grind — exactly what we wanted to avoid.
