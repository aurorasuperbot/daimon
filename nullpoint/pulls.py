...
write fails, the currency is already gone — but the next `np mine status`
...
from nullpoint import collection as _collection_mod
from nullpoint.collection import (
    Serial,
    append_serial,
    new_serial,
)
from nullpoint.identity import Identity, load_identity
from nullpoint.mining import ledger as _ledger_mod
from nullpoint.mining.formula import PULL_COST
...
def perform_pull(
    *,
    ...
    """Execute one pull. Raises:
      - FileNotFoundError: no identity (`np init` not run)
