...
Identity = ed25519 private + public key. Stored at:
  ~/.config/nullpoint/identity.key   (private, 0600)
  ~/.config/nullpoint/identity.pub   (public)
  ~/.config/nullpoint/identity.json  (metadata)
...
def generate_identity(force: bool = False) -> Identity:
    ...
    """
    from nullpoint.identity.bip39 import generate_seed_with_mnemonic
...
def load_identity() -> Identity:
    ...
    if not PRIVATE_KEY_PATH.exists():
        raise FileNotFoundError(
            f"No identity at {PRIVATE_KEY_PATH}. Run `np init` first."
        )
...
def restore_from_mnemonic(mnemonic: str, force: bool = False) -> Identity:
    ...
    from nullpoint.identity.bip39 import seed_from_mnemonic
