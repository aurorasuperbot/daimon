...
Layout (80 cols × 27 rows, each cell CELL_W × CELL_H):
  row 0                top border + inline title (`┌──── NULLPOINT ... ────┐`)
...
from nullpoint.play.card_tile import (
    CardTileInfo,
...
from nullpoint.play.frame import BattleFrame, CardState, ConnectionLine
from nullpoint.play.schema import Match, Outcome, Side, TEAM_POSITIONS
...
def render_frame_to_png(
    ...
    *,
    ...
    title_text = title_override or (
        f"NULLPOINT — Match #{frame.match_id[:4]}  vs  {frame.opponent_name}"
    )
...
def render_lineup_to_png(
    ...
    *,
    ...
    _draw_outer_frame(img, draw,
                      f"NULLPOINT — Match #{match.match_id[:4]}  ·  Lineup")
...
def render_round_banner_to_png(
    ...
    *,
    ...
    _draw_outer_frame(
        img, draw,
        f"NULLPOINT — Match #{match.match_id[:4]} vs {match.participants['opponent'].name}",
    )
...
def render_outcome_to_png(
    ...
    *,
    ...
    _draw_outer_frame(
        img, draw,
        f"NULLPOINT — Match #{match.match_id[:4]}  ·  complete",
    )

    # Big outcome banner
    banner_text = "VICTORY" if winner_is_player else "DEFEAT"
    banner_color = HP_BAR_FULL if winner_is_player else HP_BAR_LOW
