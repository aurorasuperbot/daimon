...
Under the hood, tile rendering delegates to `nullpoint.render.compose.compose_card`
...
Engineered to be replaceable:
  ...
  rarity palette lookup (nullpoint.render.compose.palette_for) ARE shared. Treat
...
from nullpoint.engine.types import Card as EngineCard
from nullpoint.engine.types import Element as EngineElement
from nullpoint.play.frame import ActiveEffect, CardState
from nullpoint.play.schema import Element as PlayElement, LoadoutCard
from nullpoint.render.compose import (
    DEFAULT_H,
    DEFAULT_W,
    CardRenderInfo,
    compose_card,
...
_PLACEHOLDER_ENV = "NULLPOINT_PLACEHOLDER_ART"
...
def _overlay_flash_border(
    ...
    """Paint a soft colored outer glow around the tile to signal 'this card just
    fired an effect'. Deliberately NOT a solid border overlay — earlier versions
    stamped a thick opaque stroke on top of the compose-layer rarity frame, which
    washed out the gold on legendary cards. Now we paint a blurred halo outside
    the card plus a thin pulse on the inner edge, preserving the rarity color.

    The halo is implemented as:
      1. A wide blurred outline (the "aura")
      2. A faint thin inner ring that accents the corners without hiding them
    Intensity fades the alpha so the effect pulses across the 300ms flash window.
    """
    rgba = _EFFECT_RGBA.get(color, _EFFECT_RGBA["red"])
    base_alpha = max(0, min(170, int(170 * intensity)))   # capped well below opaque
    w, h = img.size

    # 1) Outer aura — blurred, no sharp edges
    halo = Image.new("RGBA", img.size, (0, 0, 0, 0))
    hd = ImageDraw.Draw(halo)
    aura_thick = max(3, min(w, h) // 18)
    for i in range(aura_thick):
        # gentler alpha falloff: thicker outer edge, fading inward
        a = int(base_alpha * (1 - i / max(1, aura_thick)) * 0.9)
        hd.rectangle([i, i, w - 1 - i, h - 1 - i], outline=(*rgba, a), width=1)
    halo = halo.filter(ImageFilter.GaussianBlur(radius=max(3, aura_thick // 2)))

    # 2) Thin accent ring just inside the frame (stays narrow, no washout)
    ring = Image.new("RGBA", img.size, (0, 0, 0, 0))
    rd = ImageDraw.Draw(ring)
    inset = max(1, min(w, h) // 60)
    rd.rectangle(
        [inset, inset, w - 1 - inset, h - 1 - inset],
        outline=(*rgba, min(130, base_alpha)),
        width=1,
    )
