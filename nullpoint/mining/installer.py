...
`PostToolUse` hook that pipes the event JSON into `np mine receipt`.
        ...
        "PostToolUse": [
          {
            "matcher": ".*",
            "hooks": [
              {"type": "command", "command": "np mine receipt"}
            ]
          }
        ]
      }
...
We mark our entry with `_owner: "nullpoint"` so we can find/remove it later
...
HOOK_OWNER = "nullpoint"
DEFAULT_HOOK_COMMAND = "np mine receipt"
DEFAULT_MATCHER = ".*"
...
def _has_nullpoint_hook(post_tool_use: list) -> bool:
...
def install_hook(
    *,
    ...
    """Install the NULLPOINT hook into the given settings file.
    ...
    if _has_nullpoint_hook(post_tool_use):
        return {
...
def uninstall_hook(
    *,
    ...
    """Remove the NULLPOINT-owned hook from the settings file.
...
def hook_status(settings_path: Path = DEFAULT_SETTINGS_PATH) -> Dict[str, Any]:
    ...
    return {
        "installed": _has_nullpoint_hook(post),
        "settings_path": str(settings_path),
    }
