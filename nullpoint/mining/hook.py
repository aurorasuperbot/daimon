...
from nullpoint.mining import ledger as _ledger_mod
...
def process_event(event: Dict[str, Any],
    ...
    if tool_name.startswith("mcp__") and "nullpoint" in tool_name.lower():
        return {"action": "skipped", "reward": 0, "tool_name": tool_name,
    ...
    try:
        ...
        return {"action": "error", "reward": 0, "tool_name": tool_name,
                "reason": f"no identity ({e}); run `np init`"}
...
def main(argv: Optional[list] = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="np mine receipt")
    ...
    try:
        ...
        print(f"np mine receipt: stdin read failed: {e}", file=sys.stderr)
        return 0
    ...
    try:
        ...
        print(f"np mine receipt: bad JSON: {e}", file=sys.stderr)
        if args.verbose:
    ...
    try:
        ...
        print(f"np mine receipt: unhandled {type(e).__name__}: {e}",
              file=sys.stderr)
        if args.verbose:
