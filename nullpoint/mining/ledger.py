...
The ledger is the single source of truth for an agent's NULLPOINT balance.
...
Storage: JSONL at `~/.config/nullpoint/mining_ledger.jsonl`. One JSON object
...
constraint for V1 — the ledger lives in `~/.config/nullpoint/`, one identity
per machine.
...
from nullpoint.identity import Identity, load_identity, verify
